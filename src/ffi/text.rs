...
pub unsafe extern fn cyfile_text_with_content_and_comment(content: *const c_char, comment: *const c_char) -> *mut Text {
  Box::into_raw(Box::new(Text::with_content_and_comment(CStr::from_ptr(content).to_string_lossy().as_ref(), CStr::from_ptr(comment).to_string_lossy().as_ref())))
}

#[no_mangle]
pub unsafe extern fn cyfile_text_with_content(content: *const c_char) -> *mut Text {
  Box::into_raw(Box::new(Text::with_content(CStr::from_ptr(content).to_string_lossy().as_ref())))
}

#[no_mangle]
pub unsafe extern fn cyfile_text_with_comment(comment: *const c_char) -> *mut Text {
  Box::into_raw(Box::new(Text::with_comment(CStr::from_ptr(comment).to_string_lossy().as_ref())))
}
...
#[no_mangle]
pub unsafe extern fn cyfile_text_content(text: *mut Text) -> *mut c_char {
  CString::new((*text).content()).unwrap().into_raw()
}

#[no_mangle]
pub unsafe extern fn cyfile_text_comment(text: *mut Text) -> *mut c_char {
